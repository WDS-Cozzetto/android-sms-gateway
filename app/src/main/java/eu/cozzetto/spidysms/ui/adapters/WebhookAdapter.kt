package eu.cozzetto.spidysms.ui.adapters
...
import eu.cozzetto.spidysms.databinding.ItemWebhookBinding
        ...
        fun bind(webhook: WebHookDTO) {
            binding.apply {
                ...
                sourceText.text = when (webhook.source) {
                    eu.cozzetto.spidysms.domain.EntitySource.Local -> binding.root.context.getString(
                        R.string.local
                    )

                    eu.cozzetto.spidysms.domain.EntitySource.Gateway,
                    eu.cozzetto.spidysms.domain.EntitySource.Cloud -> binding.root.context.getString(
                        R.string.cloud
                    )
                }
                sourceIcon.setImageResource(
                    when (webhook.source) {
                        eu.cozzetto.spidysms.domain.EntitySource.Local -> R.drawable.ic_local_server
                        eu.cozzetto.spidysms.domain.EntitySource.Cloud,
                        eu.cozzetto.spidysms.domain.EntitySource.Gateway -> R.drawable.ic_cloud_server
                    }
                )
            }
        }
